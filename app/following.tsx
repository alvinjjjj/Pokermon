 ...
 * Following list page
 *
 * Shows users that the target user is following (default: current user).
...
export default function FollowingScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // 3. which of these am I (the viewer) following?
      //    If viewing my own list, all of them. Otherwise intersect.
      if (meId === tId) {
      ...
      if (__DEV__) console.error('[Following] load error:', e);
    } finally {
      setLoading(false);
    }
  }, [params.userId]);
  ...
  const toggleFollow = async (uid: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (__DEV__) console.error('[Following] follow error:', error);
      }
    } finally {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <Text style={styles.emptyTitle}>{t('profile.noFollowing')}</Text>
        </View>
      ) : (
        <FlatList
          data={rows}
